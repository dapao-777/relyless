...
test('catalog exposes the complete unique supported LLM provider set',()=>{
  ...
  expect(API_PROVIDERS.map(provider=>provider.id)).toEqual([
    'openai','deepseek','google','anthropic','xai','openai-compatible','open-responses','jalapenocloud','atlascloud','openrouter','minimax','siliconflow','tensdaq','azure','bedrock','groq','deepinfra','mistral','togetherai','cohere','fireworks','cerebras','replicate','perplexity','vercel','ollama','volcengine','alibaba','moonshotai','stepfun','huggingface',
  ]);
