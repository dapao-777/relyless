...
**About the "On-device model" channel**: when you select "On-device model (Gemini Nano)" as the provider, short word/phrase hints are generated by Chrome's built-in model on your device and **no text is sent to any server**. Note that this channel only covers short hints — batch assistance, page translation, follow-up questions, and other requests fall back to a configured ready API service or an authenticated subscription, and that text is still sent per the table above. When no fallback service is available, those features fail with an explicit error rather than silently sending data.
