...
- **Documentation-site hint settings**: your hint keyword list and the origins of sites where you chose “Don't hint again” are stored in local extension settings; no browsing URLs are stored.
...
- **Documentation-site hints**: off by default; when enabled, the extension locally checks the hostname of each page you open against your keyword segments to show a hint on the extension icon. URLs are never stored or sent; only the origins of sites you choose to dismiss are saved in local settings.
## 4. Data Categories
