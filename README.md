...
它的默认目标不是把整页替换成中文：自动辅助只面向能可靠识别的主要阅读区域，普通页面导航、控件和无关内容不会作为翻译对象。紧急整页翻译是需要单独确认的例外模式，会分批处理当前页可读正文；结果仍取决于所选模型，可能超时、失败或不完整。

> 当前版本：`v0.1.0` prerelease。尚未发布到 Chrome Web Store 或 Microsoft Edge Add-ons。
...
1. 从 GitHub Releases 下载 `relyless-0.1.0.zip`。
2. 解压后会得到 `relyless-0.1.0/`，其中包含：
   - `extension/`
   - `connector/`
   - `README.md`
   - `LICENSE`
...
- 主动翻译选段或视频内容时，会发送所选片段及有限上下文。视频的播放、展开和滚动本身不会触发模型调用。
- 紧急整页翻译只有在单独确认后，才会分批发送当前页的可读正文。
...
`npm run release:package` 只接受干净且已经提交的 `HEAD`，生成 `dist/relyless-0.1.0.zip` 和 `dist/SHA256SUMS`。Release ZIP 只包含可运行的 `extension/`、`connector/` 和根目录说明/许可文件，不包含 `node_modules`、测试或开发工具。
