...
- 阅读记录、摘要与远程个性化分析需明确开启；本地求助词档案默认开启，可关闭、导出或清理。
...
- 简短查词可完全交给本机 Gemini Nano（不联网、不计费），其余请求自动回落已配置的就绪服务。
- 释义与本页译文按内容哈希缓存在本机，换页重查不重复消耗；可为服务配置故障转移链，也可设月度 token 预算（超限需二次确认）。
...
3. 遇到生词：按住当前查词键（默认 `D`）并单击原词。解释卡片支持局部中文说明、本句翻译和针对当前语境继续追问。
4. 词都认识但句子读不顺：在弹窗开启 **阅读解构**，先看主谓宾，再展开修饰部分。
5. 只翻译眼前内容：选中英文后点击「翻译所选」，或按 `Alt+Shift+R`；选中单词时会取所在段落。
6. 复制不带词注和译文的原文：鼠标停在段落上按 `Alt+Shift+C`，或使用右键菜单。
7. 需要整页辅助：点弹窗中的 **翻译本页**，阅读数据与费用提示后确认。`Alt+Shift+T` 只打开同一确认入口。

`Alt+Shift+S` 用于开启或暂停当前页面。快捷键可在 `chrome://extensions/shortcuts` 或 `edge://extensions/shortcuts` 修改。详细场景说明见扩展内 **设置 → 使用说明**。
