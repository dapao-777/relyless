...
els.historyExport.addEventListener('click',async()=>{try{const data=await request('HISTORY_EXPORT');downloadJson(data,`relyless-history-${new Date().toISOString().slice(0,10)}.json`);setResult(els.historyActionResult,'完整记录已导出。');}catch(error){setResult(els.historyActionResult,errorText(error),true);}});
