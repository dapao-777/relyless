...
const optionsSourceLabels = {personalized:'个性化倾向',manual:'当前页手动','site-user':'个人站点规则',global:'全局固定','site-built-in':'内置站点规则','local-model':'本地模型',chatgpt:'ChatGPT 增强',grok:'Grok 增强',antigravity:'Google 增强',api:'自定义 API',general:'通用回退'};
...
function optionsProviderKind() {
  const kind = optionsState?.settings?.providerKind;
  return kind === 'api' || kind === 'grok' || kind === 'antigravity' ? kind : 'chatgpt';
}
function optionsSubscriptionLabel(kind = optionsProviderKind()) {
  if (kind === 'grok') return 'Grok';
  if (kind === 'antigravity') return 'Google';
  return 'ChatGPT';
}
function optionsInstallCommand() {
  const kind = optionsProviderKind();
  const backendFlag = kind === 'grok' ? ' --backend grok' : kind === 'antigravity' ? ' --backend antigravity' : '';
  return `node connector/install.mjs --extension-id ${chrome.runtime.id}${backendFlag}`;
}
...
function optionsRenderSubscription(){const subscription=optionsState?.subscription||{};const connected=Boolean(subscription.connected),authenticated=Boolean(subscription.authenticated),pending=Boolean(subscription.loginPending);const kind=optionsProviderKind();const label=optionsSubscriptionLabel(kind);optionsEls.subscriptionDot.classList.toggle('active',authenticated);optionsEls.subscriptionDot.classList.toggle('connected',connected&&!authenticated);optionsEls.subscriptionAccount.hidden=!authenticated;optionsEls.subscriptionEmail.textContent=subscription.email||'未提供';optionsEls.subscriptionPlan.textContent=subscription.plan||'未提供';if(!connected){optionsEls.subscriptionState.textContent='本机连接器未连接';optionsEls.subscriptionDetail.textContent=subscription.error||'请按下方步骤安装连接器。';}else if(authenticated){optionsEls.subscriptionState.textContent=`${label} 已登录`;optionsEls.subscriptionDetail.textContent=subscription.error||'当前账户可用于英文线索与局部中文说明。';}else if(pending){optionsEls.subscriptionState.textContent=`等待 ${label} 登录`;optionsEls.subscriptionDetail.textContent=subscription.error||'请完成刚打开的登录流程。';}else{optionsEls.subscriptionState.textContent='连接器已就绪';optionsEls.subscriptionDetail.textContent=subscription.error||'登录后即可使用订阅权益。';}optionsRenderModels(optionsEls.subscriptionModel,optionsState?.settings?.subscriptionModel||'',optionsEls.subscriptionModelNote);optionsEls.loginSubscription.textContent=`使用 ${label} 登录`;optionsEls.loginSubscription.hidden=!connected||authenticated||pending;optionsEls.cancelSubscription.hidden=!pending;optionsEls.logoutSubscription.hidden=!authenticated;optionsEls.loginSubscription.disabled=optionsSubscriptionBusy||!connected;optionsEls.cancelSubscription.disabled=optionsSubscriptionBusy;optionsEls.logoutSubscription.disabled=optionsSubscriptionBusy;optionsEls.refreshSubscription.disabled=optionsSubscriptionBusy;optionsEls.subscriptionModel.disabled=optionsSubscriptionBusy||!authenticated;optionsEls.testSubscription.disabled=optionsSubscriptionBusy||kind==='api'||!optionsState?.providerConfigured;
  ...
  optionsEls.installCommand.textContent=optionsInstallCommand();
}
...
function optionsRenderProvider(){const kind=optionsProviderKind(),radio=document.querySelector('input[name="provider-kind"][value="'+kind+'"]');if(radio)radio.checked=true;
  optionsEls.subscriptionPanel.hidden=kind==='api';optionsEls.apiPanel.hidden=kind!=='api';optionsRenderSubscription();
...
async function optionsSaveDetection(){const mode=document.querySelector('input[name="domain-detection-mode"]:checked')?.value||'local',before=structuredClone(optionsState.settings),current=optionsDetectionSettings(),useTranslationApi=optionsEls.detectionUseTranslationApi.checked;let api=current.api||{baseUrl:'https://api.openai.com/v1',apiKey:''},parsed=null,granted=false,jevGranted=false,jevParsed=null;try{if(mode==='api'&&!useTranslationApi){parsed=optionsParseProviderURL(optionsEls.detectionApiUrl.value);const entered=optionsEls.detectionApiKey.value.trim();if(entered.length>4096)throw new Error('识别 API Key 过长。');api={baseUrl:parsed.baseUrl,apiKey:entered||(optionsOriginPattern(api.baseUrl)===`${parsed.url.origin}/*`?api.apiKey||'':'')};if(!optionsEls.detectionApiModel.value.trim())throw new Error('请填写识别模型。');granted=await optionsEnsurePermission(`${parsed.url.origin}/*`,Boolean(api.apiKey));}let jevBaseUrl=current.jevBaseUrl||'https://router.requesty.ai/v1',jevModel=optionsEls.detectionJevModel.value.trim()||current.jevModel||'typesafe/jev-1.13.0',jevApiKey=current.jevApiKey||'';
    if(mode==='jev'){
      ...
      const enteredJev=optionsEls.detectionJevKey.value.trim();
      if(enteredJev.length>4096)throw new Error('Jev API Key 过长。');
      jevBaseUrl=jevParsed.baseUrl;jevApiKey=enteredJev||(optionsOriginPattern(current.jevBaseUrl)===`${jevParsed.url.origin}/*`?current.jevApiKey||'':'');
      jevGranted=await optionsEnsurePermission(`${jevParsed.url.origin}/*`,Boolean(jevApiKey));
    }
    const saved=await optionsSavePatch({domainDetection:{mode,subscriptionModel:optionsEls.detectionSubscriptionModel.value,apiModel:optionsEls.detectionApiModel.value.trim(),useTranslationApi,api,jevModel,jevBaseUrl,jevApiKey}},'领域识别设置已保存');if(!saved){if(granted)await chrome.permissions.remove({origins:[`${parsed.url.origin}/*`]});if(jevGranted&&jevParsed)await chrome.permissions.remove({origins:[`${jevParsed.url.origin}/*`]});return;}optionsDetectionDirty=false;await optionsRemoveUnusedPermissions(before,optionsState.settings);optionsRenderDetection();}catch(error){optionsShowError(error);}}
...
document.querySelectorAll('input[name="domain-detection-mode"]').forEach(input=>input.addEventListener('change',()=>{optionsDetectionDirty=true;optionsEls.detectionChatgpt.hidden=!['chatgpt','grok','antigravity'].includes(input.value);optionsEls.detectionApi.hidden=input.value!=='api';}));
...
document.querySelectorAll('input[name="provider-kind"]').forEach(input=>input.addEventListener('change',async()=>{const kind=input.value;const label=kind==='api'?'API':optionsSubscriptionLabel(kind)+' 订阅';if(await optionsSavePatch({providerKind:kind},'已切换至 '+label)){if(kind!=='api')await optionsRefreshSubscription();}}));
