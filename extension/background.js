...
import { analyze, analyzeBatch, englishTokenStats, historyMatches, identifyPageLanguage, isKnownTerm, lemma, localReferenceFor, resolveCanonicalTerm } from './lexicon.js';
import { encounter, familyStage, interact, migrateSupportWord, normalizeKnownAt, normalizeSenseLabel, readingEvidence } from './reading.js';
...
const DATA_MUTATIONS=new Set([...HISTORY_MUTATIONS].filter(type=>type!=='PERSONALIZATION_ANALYZE').concat(['STATE_PATCH','AUTOMATION_PATCH','VIDEO_SETTINGS_PATCH','ASSIST_COMMIT','ENCOUNTER','INTERACT','READING_ACTIVITY','READING_FINISHED','ON_DEMAND_SUGGESTION']));
...
function publicState(state,trusted) {
  ...
  const settings = trusted ? source : {
    ...
    helpLanguage:source.helpLanguage,lookupKey:source.lookupKey,lookupDisplay:source.lookupDisplay,hintDisplay:source.hintDisplay,fluencyHints:source.fluencyHints,keyboardNav:source.keyboardNav,
...
function validatePatch(patch,currentSettings) {
  ...
  if (patch.fluencyHints !== undefined) { if (typeof patch.fluencyHints !== 'boolean') throw new Error('无效流畅度提示设置。'); result.fluencyHints=patch.fluencyHints; }
...
async function apiRequest(provider,payload,instructions,schema,{onContent,trace,beforeRequest}={}) {
  ...
  const attempt=async current=>{
    ...
    try {
      ...
      providerError='';
      void recordModelUsage({provider:'api',service:current.name||current.model,model:current.model,operation:trace?.operation||'',ok:true,usage:usageReport,inputChars,outputChars:JSON.stringify(output).length,inputText:JSON.stringify(payload),outputText:JSON.stringify(output)});
      return output;
    } catch(error) {
      void recordModelUsage({provider:'api',service:current.name||current.model,model:current.model,operation:trace?.operation||'',ok:false,usage:usageReport,inputChars,inputText:JSON.stringify(payload)});
      let reported=error;
...
async function refreshRequestedDefinitions(items,decisions,state,expectedProvider,source){
  ...
  await mutate(async current=>{
    ...
    for(const item of items){
      ...
      const senses=index<0?[...word.senses,{key:senseKey,label,stage:'hint',locked:false,lastHelpAt:0,opportunityDays:0,quietUntil:0,quietCycles:0,quietOpportunityDays:0,hintPreference:null,peekCount:0,assistedPageKey:'',definition,...(resolved.embedding?{embedding:resolved.embedding}:{})}]:word.senses.map((sense,senseIndex)=>senseIndex===index?{...sense,label,definition}:sense);
...
async function handle(message,sender) {
  ...
  switch(message.type){
    ...
    case 'READING_FINISHED':return readingHistory.finish(message,sender);
