...
async function apiRequest(provider,payload,instructions,schema,{onContent,trace,beforeRequest}={}) {
  ...
  try {
    ...
    providerError='';
    if(!trace?.incognito)void recordModelUsage({provider:'api',service:service.name||service.model,model:service.model,operation:trace?.operation||'',ok:true,usage:usageReport,inputChars,outputChars:JSON.stringify(output).length});
    return output;
  } catch(error) {
    if(!trace?.incognito)void recordModelUsage({provider:'api',service:service.name||service.model,model:service.model,operation:trace?.operation||'',ok:false,usage:usageReport,inputChars});
    let reported=error;
...
async function providerOperation(operation,trace,model='',provider='chatgpt',inputChars=0){return diagnostics.provider(trace,provider,model,provider,async()=>{const service=SUBSCRIPTION_SERVICE_LABELS[provider]||'订阅服务';try{const result=await operation();providerError='';if(!trace?.incognito)void recordModelUsage({provider,service,model:model||'默认模型',operation:trace?.operation||'',ok:true,inputChars,outputChars:JSON.stringify(result??null).length});return result;}catch(error){providerError=error.message||'服务连接失败。';if(!trace?.incognito)void recordModelUsage({provider,service,model:model||'默认模型',operation:trace?.operation||'',ok:false,inputChars});throw error;}});}
