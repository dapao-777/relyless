...
import { ENGLISH_FREQUENCY_RANK } from './frequency/english-frequency.js';
...
function frequency(word) {
  ...
  for (const lemma of morphologyCandidates(word)) {
    const rank = ENGLISH_FREQUENCY_RANK.get(lemma);
...
function knownTermKey(surface) {
  ...
  const forms = morphologyCandidates(term,false).filter(form => ENGLISH_FREQUENCY_RANK.has(form));
